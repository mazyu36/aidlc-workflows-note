...
awslabs/aidlc-workflows v2（基点コミットは docs/aidlc-workflows/meta.json の analyzedCommit）の `docs/` 配下 Markdown を、構造を 1 対 1 で保った日本語版 Markdown にする。原文は `/tmp/aidlc-v2/docs/<相対パス>`、出力は本リポジトリの `mirror-src/<同じ相対パス>`（ファイル名・拡張子も同一。スペースを含む名前もそのまま）。
