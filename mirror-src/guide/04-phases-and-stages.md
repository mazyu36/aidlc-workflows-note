...
**stage の色:** 緑 = ALWAYS（選んだ scope に含まれる限り常に実行）。黄 = CONDITIONAL（scope・project type・plan に応じてスキップされる場合がある）。scope ごとの厳密な stage 所属は [Stage-by-Scope Matrix](05-scopes-and-depth.md#stage-by-scope-matrix) を参照。
...
- [Scope・Depth・テスト戦略](05-scopes-and-depth.md) — scope がどの stage を実行するかを制御する仕組み、完全な [Stage-by-Scope Matrix](05-scopes-and-depth.md#stage-by-scope-matrix) を含む
