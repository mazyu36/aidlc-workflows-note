...
リポジトリごとに 1 つのストアである。当てはまる brownfield intent ごとに、stage は
まずストアの記録済み scope と working-tree フィンガープリントを確認する。検証済みで
最新のストアで、そのカバレッジが intent に合っているものは人の選択によって再利用できる。
それ以外の場合は full または focused なスキャンがその 9 ファイルを上書きする
...
| Stage | 主要成果物 | 条件 |
|-------|--------------|-----------|
| 1.1 Intent Capture | `intent-capture-questions.md`（source register と確認済みの回答）、`intent-statement.md`、`stakeholder-map.md`（インライン source タグと必須の assumption セクション） | 常に |
...
| Stage | 主要成果物 | 条件 |
|-------|--------------|-----------|
| 2.1 Reverse Engineering | `architecture.md`・`code-structure.md`・`technology-stack.md` を含む 9 ファイル（space レベルの `aidlc/spaces/<active-space>/codekb/<repo>/` へ書かれる — リポジトリごとに 1 つの共有ストアで、検証済みで最新なら再利用され、人が選んだ再スキャンで置き換えられる。intent の記録に入るのは stage の `memory.md` 日誌だけ） | brownfield のみ |
