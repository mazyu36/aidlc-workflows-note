...
| **Hook** | イベントに応じて Claude Code が自動実行する TypeScript スクリプト。この実装は 14 の hook を使い、すべて `settings.json` にプロジェクト全体で登録される: ワークフローの背骨（audit ログ、sensor ディスパッチ、runtime グラフのコンパイル、statusline 同期、コンパクション時の状態検証、subagent 追跡、ターン終端のループ強制）に加え、セッションライフサイクル、human-turn の記録、厳密な dispatch-rule 配送、状態遷移と reviewer スコープのガード、statusline コマンド。それぞれ自己判定し、ワークフローが無ければ no-op になる。 |
