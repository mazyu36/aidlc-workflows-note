...
| hook の存在 | `settings.json` が束線するすべての hook（`hooks` ブロック + `statusLine` コマンド — フレームワークの全 16 hook）が `.claude/hooks/` に存在すること。束線済みなのに欠けている hook は大きく失敗する。期待一覧を `settings.json` から取るため、そこに hook を足せば自動で検査対象になる |
