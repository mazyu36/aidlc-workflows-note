...
1. gate を提示する前に、`report --result awaiting-approval` が保留中の gate を記録する（`STAGE_AWAITING_APPROVAL`）
...
(2) 応答の後 — 正確なユーザー入力を伴って `approved` か `rejected` を報告する。
report が所有するライフサイクルイベントが gate の完全な audit 記録である。
`aidlc-log.ts decision` や `aidlc-log.ts answer` を呼ばないこと。

**gate 以外の各質問インタラクションで:** 回答を受け取った後 — `aidlc-log.ts answer` を
通じて Q&A サマリを追記する。
