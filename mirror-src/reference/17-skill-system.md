...
engine は `core/tools/aidlc-orchestrate.ts` に著述され、各 harness に `<harness-dir>/tools/aidlc-orchestrate.ts`（例: `.claude/tools/`）として出荷される; それは正確に 4 つのサブコマンド `next`、`continue`、`report`、`park` を持つ Bun CLI である。`continue` は内部の steering 用トランスポートである。
...
`aidlc-directive.ts` は、`kind` フィールドをキーとする **10 個** の directive kind に対する discriminated union を定義する。各 directive はその kind が必要とするフィールドを正確に運び、kind ごとの許可キー集合で強制される（その kind の集合の外のフィールドは未知キーとして拒否される）。engine は **今日 8 つの kind を発する**; 2 つは、後のウェーブがそれらを配線するまでループを complete の形に保つ、文書化されたプレースホルダである。

| `kind` | 今日発する？ | conductor がすること |
|--------|----------------|--------------------------|
| `print` | Yes | `directive.message` が言うことを正確に行う — それが権威である。3 つの形状: **terminal**（status/help/doctor/version のような読み取り専用ユーティリティを名指す; それを走らせ、stdout を逐語で表示し、STOP）、**run-then-continue**（scope change、jump `execute`、または fresh な workspace での `intent-create` のような変異ツールを名指す; それを走らせ、それからステップ 1 に戻る）、そして **run-then-stop**（確認済みの `--new-intent` の作成; `intent-create` を走らせ、STOP し、`next` を再実行せずに harness 固有の fresh-session フローを使って引き渡す）。変異は名指されたツールに住み、`next` には決して住まない。 |
...
| `load-steering` | Yes | `rules_content` を順に適用し、それをアクティブな stage の rule bundle として保持し、直ちに `aidlc-orchestrate continue <continue_token>` を呼ぶ。チャンクの進捗を報告したり語ったりしない。 |
| `run-stage` | Yes | 先行する `load-steering` シーケンスがアクティブ space のすべての実質的な rule を届けた。`inline_context_paths` を、届いたコンテンツではなくブロッキングなロードリストとして扱う: 必須の各エントリを明示的に読み、stage ファイル/consumes を読む前、diary を初期化する前、本体を走らせる前、mob の support をディスパッチする前、または成果物を書く前に、その内容を待つ。mob はまずその lead ペルソナをロードする。`context_warnings` があれば示し、読み取り可能なペルソナ/knowledge ロースターで続行する。ディスパッチされたトポロジでは、積み上げられた rule bundle を逐語的にすべての agent brief に貼り付ける。それから stage ボディを走らせ、`produces` を書き、`directive.memory_path` で diary を保ち、`directive.gate` の前に任意の `directive.single` で分岐する（[Orchestrator](03-orchestrator.md) を参照）。directive はまた解決済みのルーティングフィールドをグラフノードから直接運ぶ。 |
| `ask` | Yes | `directive.question` を harness の構造化された質問サーフェス経由でレンダーし、その型付き応答契約に従う。通常の ask は `report --user-input` を通じて戻る; `ask_type: "new-work-routing"` は `response_route: "next"`、`new_work_description`、`proposed_scope` を運ぶので、その continue / separate-work / reshape の回答は `next` を経由してルーティングされ、`report` を経由することは決してない。engine は人間に自ら尋ねることは決してない — ターンを conductor に委ねる。 |
...
図のテキスト記述: `next`（`$ARGUMENTS` を逐語で渡す）が 1 つの directive を返す。conductor は `directive.kind` で分岐する。`run-stage`、通常の `ask`、`invoke-swarm`、および run-then-continue の `print` directive では、名指された一手を実行し `report` を呼び、それが `next` にループバックする。型付きの `new-work-routing` ask は、代わりにその宣言された `next` ルートを通ってループする。terminal の `print`、`error`、`done` ではループを止める。
...
1. **Identity** は scope ごとに 1 ファイル、`dist/claude/.claude/scopes/aidlc-<name>.md` に住む — frontmatter（必須の `name` と `depth`、加えて任意の `keywords`、`description`、`testStrategy`、`review_cap`、`runner`）に scope を記述する散文を加えたもの。出荷される集合は `bugfix`、`enterprise`、`feature`、`infra`、`mvp`、`poc`、`refactor`、`security-patch`、`workshop`。
...
これら 6 つの `SWARM_*` イベントは 82 イベントの audit 分類体系の一部である（[State Machine](12-state-machine.md) を参照）。exit-2 のエンベロープでは conductor が baton を取り戻す - 失敗は autonomy モードに関わらず常に halt して人間を再び関与させる。
...
- **`report` がコミットする transition** - workflow / phase / stage の machine と 82 イベントの audit 分類体系。[State Machine](12-state-machine.md) を参照。
