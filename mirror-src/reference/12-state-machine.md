...
| Event | Emitter | 備考 |
|---|---|---|
| `DECISION_RECORDED` | `tools/aidlc-log.ts` | gate 以外の `AskUserQuestion` の前に発火し、選択肢を捉える |
| `QUESTION_ANSWERED` | `tools/aidlc-log.ts` | gate 以外の質問への応答の後に発火する; 承認の選択は `report` が所有するライフサイクルイベントである |
