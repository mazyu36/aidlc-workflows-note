...
| `intent-capture` | `[claim-sources, required-sections, upstream-coverage]`（`claim-sources` は、stage の成果物にわたって、見えるインラインの provenance・正式な source-register の値・人間が確認した assumption の厳密一致をチェックする） |
